...
export default class Group {
	...
	async getAllUsers (noDuplicates = false) {
		...
		const stmt = CR.db.users.prepare(`select user_id, args, email, password, activation_key, activation_key_time from users_groups inner join users on users_groups.user_id = users.id where group_id in (${params}) and enabled = 1 and \`from\` <= @current_time and (\`to\` is null or \`to\` > @current_time)`);
